...
const AddEditInforDoctor = ({ t }) => {
  ...
  useEffect(() => {
    const getListSpecialty = async () => {
      try {
        ...
        alert('Faild to get list of specialty');
  ...
  const saveInforDoctor = async () => {
    ...
    try {
      ...
      if (res && res.errCode === 0) {
        dispatch(
          addSuccessMessage({
            ...
            content: 'Thêm thành công thông tin bác sĩ',
          })
        );
        ...
        dispatch(
          addWarningMessage({
            title: 'Thêm thất bại',
            content:
              'Vui lòng kiểm tra lại thông tin và điền đầy đủ thông tin mô tả bác sĩ!!!',
          })
        );
        srollToInput();
      }
      ...
      setLoading(false);
      alert('Faild to add informations doctor');
  ...
  useMemo(() => {
    ...
    const getInforDoctor = async () => {
      try {
        ...
        alert('Faild to get inforDoctor');
    ...
    <ManagerLayout>
      ...
      <form ref={formref}>
        <div className="px-12 flex gap-[10%]">
          ...
          <div className="w-[65%] mb-6">
            <label className="font-bold text-[20px] inline-block">
              {t('addinfordoctor.inforIntroduce')}
            </label>
            <span className="text-red-600">*</span>
            {error.description && (
        ...
        <div className="px-12 flex gap-[15%]">
          <div className="mb-4 w-[25%]">
            <label className="font-bold text-[20px]">
              {t('addinfordoctor.namespecialty')}
            </label>
            <span className="text-red-600">*</span>
            <select
            ...
            >
              <option value={0}>{`--- ${t(
                'addinfordoctor.selecspecialty'
              )} ---`}</option>
              {listSpecialty.map((option) => (
                <option
                  value={option.id}
                  key={option.id}
                  // selected={option.value === selectedRole}
            ...
            {error.selectedSpecialty && (
              <span className="text-red-600">{error.selectedSpecialty}</span>
            )}
          </div>
          <div className="mb-4">
            <label className="font-bold text-[20px]">
              {t('addinfordoctor.price')}
            </label>
            <span className="text-red-600">*</span>
            <select
              onChange={(e) => {
            ...
            >
              {PRICES_OPTIONS.map((option) => (
                ...
                >
                  {language === 'vi'
                    ? getFormattedPriceVND(option.label.vi)
                    : getFormattedPriceUSD(option.label.en)}
                </option>
              ))}
            </select>
          </div>
      ...
      <button
        onClick={handleAddInforDoctorOnClick}
