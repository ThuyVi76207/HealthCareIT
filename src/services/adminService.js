...
const filterNewsByDoctorId = (data) => {
  return axiosClient.get(
    `/api/filter-news-by-doctor-id?limit=${data.limit}&doctorId=${data.doctorId}`,
    data
  );
};
...
const getAllPatientStaff = (data) => {
  return axiosClient.get(
    `/api/get-list-patient-for-staff?doctorId=${data.doctorId}&date=${data.date}`,
    data
  );
};
...
const deleteMultipleBooking = (list) => {
  return axiosClient.delete(`/api/delete-multiple-booking`, {
    data: { contentIds: list },
  });
};
