...
function SendIDModal() {
  ...
  const { title, rightButtonText, patient } =
    useSelector((state) => state.sendModal) || {};
  // console.log(patient)
  ...
  useEffect(() => {
    if (patient && patient.patientData) setEmail(patient.patientData.email);
  }, [patient]);
    ...
    try {
      ...
      if (res && res.errCode === 0) {
        ...
        dispatch(
          addSuccessMessage({
            title: 'Thành công',
            content: 'Mã phòng đã được gửi đến bệnh nhân!!!!',
          })
        );
      }
    } catch (error) {
